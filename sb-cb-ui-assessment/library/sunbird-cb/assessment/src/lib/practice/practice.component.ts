...
import { WsEvents } from './../services/event.model'
import { ActivatedRoute, NavigationStart, Router } from '@angular/router'
import { CommonViewerUtilService } from '../services/viewer-util.service'
// tslint:disable-next-line
import _ from 'lodash'
import { NSQuiz } from './../quiz/quiz.model'
import { DomSanitizer, SafeHtml } from '@angular/platform-browser'
import { CommonViewerDataService } from '../services/viewer-data.service'
...
// import { CommonViewerDataService } from '../../viewer-data.service'
...
export class PracticeComponent implements OnInit, OnChanges, OnDestroy {
  ...
  selectedAssessmentCompatibilityLevel = 6
  ...
  constructor(
    ...
    private viewerSvc: CommonViewerUtilService,
    private router: Router,
    private valueSvc: ValueService,
    // private vws: CommonViewerDataService,
    public snackbar: MatSnackBar,
    private sanitized: DomSanitizer,
    private viewerDataSvc: CommonViewerDataService,
  ...
  ngOnInit() {
    ...
    if (this.widgetContentService.currentMetaData.children && this.widgetContentService.currentMetaData.children.length) {
      ...
      // console.log((window.location.href.includes('public')))
      // console.log((window.location.href.includes('author')))
      // console.log(window.location.href.includes('editMode'))
      this.forPreview = window.location.href.includes('author')
      const activeResource = this.findNested(this.widgetContentService.currentMetaData, 'identifier', this.identifier)
      // tslint:disable-next-line: max-line-length
      this.showQuestionMarks = this.widgetContentService.currentMetaData.showMarks ? this.widgetContentService.currentMetaData.showMarks : 'No'
      // this.selectedAssessmentCompatibilityLevel = item.compatibilityLevel
      // console.log('item.children', item.children)
      // console.log('selectedAssessmentCompatibilityLevel', this.selectedAssessmentCompatibilityLevel)
      // console.log('this.identifier',this.identifier, 'item.identifier', item.identifier)
  ...
  startSection(section: NSPractice.IPaperSection) {
    // console.log('quizJson', this.quizJson, this.paperSections)
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    // tslint:disable-next-line: max-line-length
    this.resourceName = this.fromCreation ? (this.quizData && this.quizData.name ? this.quizData.name : '') : this.viewerDataSvc.resource ? this.viewerDataSvc.resource.name : ''
    // console.log('this.resourceName-->', this.resourceName, this.quizData)
          ...
          if (this.timeLeft < 0) {
            if (this.paperSections && this.paperSections.length) {
              if (this.allSecAttempted.full) {
                ...
                if (!this.fromCreation || !this.forPreview) {
                  this.submitQuiz()
                }
  ...
  openSectionPopup(submitAssessment = false, getTime = true) {
    ...
    const tableColumns: any[] = [
      { header: this.fromCreation ? 'Section' : 'practiceoverview.section', key: 'section' },
      { header: this.fromCreation ? 'No of Questions' : 'practiceoverview.noOfQuestions', key: 'NoOfQuestions' },
      { header: this.fromCreation ? 'Answered' : 'practiceoverview.answered', key: 'answered' },
      { header: this.fromCreation ? 'Not Answered' : 'practiceoverview.notAnswered', key: 'notAnswered' },
      { header: this.fromCreation ? 'Marked for Review' : 'practiceoverview.markedForReview', key: 'markedForReview' },
      { header: this.fromCreation ? 'Not Visited' : 'practiceoverview.notVisited', key: 'notVisited' },
    ]
    const tableData: any = []
    /* tslint:disable */
    ...
    if (submitAssessment) {
      popupData['warningNote'] = this.fromCreation ? '' : 'practiceoverview.warningNoteForAssessmentSubmit',
        popupData['buttonsList'] = [
          {
            response: 'yes',
            text: this.fromCreation ? 'Yes' : 'practiceoverview.submitYes',
            classes: 'blue-outline',
          },
          {
            response: 'no',
            text: this.fromCreation ? 'No' : 'practiceoverview.submitNo',
            classes: 'blue-full',
          },
          // {
          //   response: 'Back',
      ...
      popupData['buttonsList'] = [
        {
          response: 'back',
          text: this.fromCreation ? 'Back' : 'practiceoverview.back',
          classes: 'gray-full',
        },
        {
          response: 'submitAssessment',
          text: this.fromCreation ? 'Submit Test' : 'practiceoverview.submitTest',
          classes: 'blue-full',
        }
      ]
    }
      ...
      popupData['buttonsList'] = [
        {
          ...
          text: this.fromCreation ? 'Next Section' : 'practiceoverview.nextSection',
          classes: 'blue-full',
        },
        {
          response: 'no',
          text: this.fromCreation ? 'Back to Assessment' : 'practiceoverview.backToAssessment',
          classes: 'gray-full',
        }
      ]
    }
